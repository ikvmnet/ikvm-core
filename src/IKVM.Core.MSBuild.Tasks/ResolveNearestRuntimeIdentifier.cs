namespace IKVM.Core.MSBuild.Tasks
{
    ...
    /// </summary>
    public class ResolveNearestRuntimeIdentifier : Task
    {
        ...
        {
            using var stream = typeof(ResolveNearestRuntimeIdentifier).Assembly.GetManifestResourceStream("runtime.json");
            using var reader = new StreamReader(stream);
            var g = SimpleJsonParser.FromJson<RuntimesElement>(reader.ReadToEnd());
            return g.Runtimes;
        }
        ...
        public override bool Execute()
        {
            if (string.IsNullOrWhiteSpace(TargetRuntimeIdentifier) == false)
                NearestRuntimeIdentifier = GetNearestRuntimeIdentififer(TargetRuntimeIdentifier, AvailableRuntimeIdentifiers.Split([';'], System.StringSplitOptions.RemoveEmptyEntries));
